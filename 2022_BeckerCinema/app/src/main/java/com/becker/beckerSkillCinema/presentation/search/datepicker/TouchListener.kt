...
import com.becker.beckerSkillCinema.utils.newDate
import com.becker.beckerSkillCinema.utils.setOnClickListener
...
class TouchListener {
    ...
    fun onTouchEvent(
        event: MotionEvent?,
        setting: SettingsPicker,
        setDateListener: onDateClickListener?,
        block: () -> Unit
    ) {
        when (event!!.action) {
            MotionEvent.ACTION_DOWN -> {
                ...
                setting.datePicker.newDate(
                    setting.viewRectF.setOnClickListener(event.x, event.y, setting.cellSizeW),
                    setting
                ) { block() }
                if (datePosition != Pair(0, 0)) {
                    if (oldDatePosition != null) setting.datePicker[oldDatePosition!!.first][oldDatePosition!!.second].status =
                        State.NOT_SELECT
